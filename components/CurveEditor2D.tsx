...
export default function CurveEditor2D({ baseParams, controlPoints, setControlPoints }: CurveEditor2DProps) {
  ...
  const drawCurve = () => {
    ...
    if (adjustedControlPoints.length >= 2) {
      ...
      // Draw wall thickness indicator (inward from profile curve)
      if (showWallThickness) {
        // Draw a dashed line representing the inner wall
        ctx.strokeStyle = "#888888" // Gray for inner wall
        ctx.lineWidth = 1
        ctx.setLineDash([3, 3])
        ctx.beginPath()

        // Calculate inner points (inward from profile)
        const innerPoints = points3D.map((point) => ({
          x: Math.max(0, point.x - baseParams.wallThickness),
          y: point.y,
        }))

        // Start at the first inner point
        ctx.moveTo(mmToPixels(innerPoints[0].x, "x"), mmToPixels(innerPoints[0].y, "y"))

        // Draw inner curve segments
        for (let i = 1; i < innerPoints.length; i++) {
          ctx.lineTo(mmToPixels(innerPoints[i].x, "x"), mmToPixels(innerPoints[i].y, "y"))
        }

        ctx.stroke()
        ctx.setLineDash([])

        // Draw wall thickness label
        ctx.fillStyle = "#888888"
        ctx.font = `${typography.fontSizes.small} ${typography.fontFamily}`
        ctx.textAlign = "left"
        ctx.textBaseline = "top"
        ctx.fillText(`WALL: ${baseParams.wallThickness}MM`, 10, 10)
      }
    }
  ...
  useEffect(() => {
    drawCurve()
  }, [baseParams, controlPoints, scale, offset, canvasSize, showWallThickness])
    ...
    <div className="w-full h-full relative" style={{ backgroundColor: colors.background }}>
      ...
      <div className="absolute bottom-4 right-4 flex space-x-2">
        <button
          ...
          style={{
            backgroundColor: showWallThickness ? colors.primary : colors.background,
            color: showWallThickness ? colors.background : colors.primary,
