...
function generateProfileCurve(controlPoints: ControlPoint[]): THREE.Vector2[] {
  try {
    ...
    if (!controlPoints || controlPoints.length < 2) {
      throw new Error("Not enough control points to generate curve")
    }
    ...
    console.error("Error generating profile curve:", error)
    // Return a simple fallback curve
    return [new THREE.Vector2(10, 0), new THREE.Vector2(10, 10), new THREE.Vector2(5, 20)]
  }
}
...
function generateOffsetCurve(profilePoints: THREE.Vector2[], wallThickness: number): THREE.Vector2[] {
  try {
    ...
    return profilePoints.map((point, i) => {
      // Determine if the normal is pointing outward or inward
      // For a typical vessel profile, we want to offset inward
      // This means subtracting the normal (multiplied by thickness) from the point
      return point.clone().sub(normals[i].clone().multiplyScalar(wallThickness))
    })
  } catch (error) {
    console.error("Error generating offset curve:", error)
    // Return a simple fallback offset curve
    return profilePoints.map((p) => new THREE.Vector2(Math.max(1, p.x - wallThickness), p.y))
  }
}
...
function createClosedProfile(outerPoints: THREE.Vector2[], innerPoints: THREE.Vector2[]): THREE.Vector2[] {
  try {
    ...
    console.error("Error creating closed profile:", error)
    // Return a simple fallback profile
    return [
...
export function generateVesselGeometry(
  ...
  try {
    // Validate input
    if (!controlPoints || controlPoints.length < 2) {
      throw new Error("Not enough control points to generate geometry")
    }
...
export function generateBaseCylinderGeometry(baseParams: BaseParameters): THREE.BufferGeometry {
  try {
    // Validate input
    if (baseParams.outerDiameter <= 0 || baseParams.height <= 0) {
